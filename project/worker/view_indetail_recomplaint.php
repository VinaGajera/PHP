    ...
    <div class="wrapper">
        <div class="wrapper_inner">
            ...
            <div class="main_container">
                ...
                <h3 style="margin:20px;color:red;">View  Re-Complaint Indetail:</h3>
                <table class="table table-borderedless mx-auto" style="width: 85%;margin-bottom:100px;">
                    <tbody>
                        ...
                        <tr>
                            <th scope="row">First the Complaint</th>
                            <td colspan="3">';
                        ...
                        <tr>
                            <th scope="row">User send Proof of ReComplaint</th>
                            <td colspan="3">';

                        $rsql="SELECT * FROM `recomplaint_tb` WHERE `c_id`='$complaint_get_id'";
                        $rresult=mysqli_query($conn,$rsql);
                        while($rrow = mysqli_fetch_assoc($rresult))
                        {
                         
                            $rres_c_image=$rrow["user_send_image"];
                            $rres_c_image=explode(" ",$rres_c_image);
                            $count=count($rres_c_image)-1;
                            for ($i=0; $i <$count ; ++$i) { 
                                echo '<img src="../../USER_WORK/'.$rres_c_image[$i].'" alt="" style="margin:20px;" height="150px" width="150px" >';
                            }
                        }
                            echo '</td>
                        </tr>';
                            ...
                            if($recever_img=="")
                                ...
                                echo '<tr>
                                    <th scope="row" style="color:red">You solve problem first Time</th>
                                    <td colspan="3">';
                                    $recever_img=explode(" ",$recever_img);
                                    $count=count($recever_img)-1;
                                    for ($i=0; $i <$count ; ++$i) { 
                                        echo '<img src="'.$recever_img[$i].'" alt="" style="margin:20px;" height="150px" width="150px" >';
                                    }
                                    echo '</td>
                                </tr>';
                                }
                                echo  '<tr>
                                <td>You Complete Work Status :</td>
                                <td>';
                                $rssql="SELECT * FROM `recomplaint_tb` WHERE `c_id`='$complaint_get_id'";
                                $rsresult=mysqli_query($conn,$rssql);
                                while($rsrow = mysqli_fetch_assoc($rsresult))
                                {

                                 
                                    $rsres_c_image=$rsrow["recomplain_solve_image"];
                                    if($rsres_c_image=="")
                                    {
                                        echo '<td colspan="2" >No Data Upaloaed....</td></tr>';
                                    }else{
                                        $rsres_c_image=explode(" ",$rsres_c_image);
                                        $count=count($rsres_c_image)-1;
                                        for ($i=0; $i <$count ; ++$i) { 
                                            echo '<img src="'.$rsres_c_image[$i].'" alt="" style="margin:20px;" height="150px" width="150px" >';
                                        }
                                    }
                                }
                                    echo '</td>
                        ...
                        echo '<tr>
                            ...
                            <td><button type="button" class="btn btn-primary" data-toggle="modal"
                                ...
                                <a href="recomplaint.php" class="btn btn-primary">Back</a></td>
  ...
  <?php
        if($_SERVER["REQUEST_METHOD"]=="POST"){
           ...
           // image store -> $data;
                    ...
                    $sql="UPDATE recomplaint_tb  SET recomplain_solve_image='$data' WHERE c_id=$complaint_get_id";
                    $result=mysqli_query($conn,$sql);
                    if($result)
                    {
                    ...
                    while($row = mysqli_fetch_assoc($result))
                    {
                        ...
                        while($row2 = mysqli_fetch_assoc($result2))
                        {
                            ...
                            $sql2="INSERT INTO `notify` ( `sender_user_id`, `complaint_id`, `sender_user_role`, `msg`, `reciver_id`, `msg_recive`, `send_msg`, `status`) VALUES('$sender_user_id', '$complaint_get_id', 'worker', '$msg', '$recever_user_id', 'admin', current_timestamp(), '0')";
                            $result2=mysqli_query($conn,$sql2);
                            if($result2)
                            {
                                echo'<script> swal({
